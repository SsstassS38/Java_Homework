...
async def mes_start(message: types.Message):
    ...
    await message.answer(f'{name}, привет! Сыграем с тобой в конфеты. Правила: Нужно взять кол-во конфет не более 29, кто последний забирает остаток конфет - победитель\n'
                         f'Для начала игры введи команду /new_game.\n'
                         f'Для изменения кол-ва конфет, для начала введи команду например /set 250, затем /new_game\n'
                         f'Также можно сыграть с человеком через команду /duel и id соперника')
...
async def mes_new_game(message: types.Message):
    ...
    if first:
        ...
        await message.answer(f'Игра началась. По жребию первым ходит умный БОТ')
    ...
    if len(duel) == 0:
        if new_game:
            if message.text.isdigit() and 0 < int(message.text) < 29:
                ...
                if total <= 0:
                    await message.answer(f'Ура! {name} красавчег, ты победил!')
                    new_game = False
                else:
        ...
        if current == int(message.from_user.id):
            ...
            if new_game:
                if message.text.isdigit() and 0 < int(message.text) < 29:
                    ...
                    if total <= 0:
                        await message.answer(f'Ура! {name}красавчег, ты победил!')
                        await dp.bot.send_message(enemy_id(), 'Бай бай лузер, попробуем еще? :)')
                        new_game = False
                    else:
                        await message.answer(f'{name} взял {message.text} конфет. '
                                             f'На столе осталось {total}')
